...
const Sections = ["Banner Section", "Brands", "Social Media Management", "Influencer Marketing", "Video Production", "Featured Marketing"];
...
const Work = () => {
  ...
  const [bannerSectionData, setBannerSectionData] = useState({ heading: '',subheading:"", description: '',subdescription:"" });
  const [brandsData, setBrandsData] = useState({ heading: '', description: '', brandsImage: [] });
  const [socialMediaManagementData, setSocialMediaManagementData] = useState([{ title: '', description: '', images: [] }]);
  const [influencerMarketingData, setInfluencerMarketingData] = useState([{ title: '', images: [] }]);
  const [videoProductionData, setVideoProductionData] = useState({ heading: '', productionImage: [] });
  const [featuredMarketingData, setFeaturedMarketingData] = useState({ heading: '', cards: [] });
          ...
          <div key={index} className='mb-5'>
            ...
            {field.type === 'input' ? (
              <input
                type="text"  
                value={
                  selectedSection === "Banner Section"
                    ? bannerSectionData?.[field.key] || ''
                    : selectedSection === "Brands"
                    ? brandsData?.[field.key] || ''
